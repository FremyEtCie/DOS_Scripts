...
Script, Dos batch call [CONV_CBx-TO-PDF.BAT](https://github.com/FremyEtCie/DOS_Scripts/tree/main/ComicBook-batch-converter) to create a PDF file from Comic Book file

## Processing

Image processing for create PDF file from Comic Book (CBR & CBZ extensions) with 7zip / ImageMagik / NConvert / Ghostscript tools on a Windows x64 bits platform
...
3. Detect pdf file and copy it into the folder .PDF/ for compression after
...
```list
 ...
 |   |   +-- fichier1_cbr_extracted/
 |   |   +-- fichier2_cbz_extracted/
 |   |   +-- listeCB.txt
 |   |   +-- PictureListToDo.txt
 +-- tools/
...
- Ghostscript
  ...
  - [ ] manage issues if exist - need to test more files
- CONV_CBx-TO-PDF.BAT
  ...
  - [x] rename files with specific charaters as "..." ".." " ." "!" "%" before/after the treatment
  ...
  - [x] delete file(s) which are not pictures files inside the CBR or CBZ file with specifical extension or filename (extensions.dat)
  - [x] detect if no file decompressed
  - [x] remove / delete old folder temporary unused after
  - [x] convert .JPG and .PNG picture files inside Comic Book file
  - [ ] manage doublon files
  - [ ] permit to compress or not pdf file
